...
class ViewController_Copy: UIViewController, ARSCNViewDelegate ,SCNAnimationProtocol{
    ...
    override func viewDidLoad() {
        ...
        let pinchGesture = UIPinchGestureRecognizer(target: self, action: #selector(scalePiece))
...
//            scaleMainNode(with: SCNVector3(0.1,0.1,0.1))
    ...
    func MoveThatPlayers(player:Players, Army:String, hasAnimations:Bool=false, completionHandler : ()->()) ->Players
    {
                ...
                if !resultsOf3dTap.isEmpty
                {
                    ...
                    case .RED:
                        ...
                        case "RED1" : playerIndex = 0
                            ...
                            redPlayerArmy[playerIndex] = MoveThatPlayers(player: redPlayerArmy[playerIndex], Army: "RED", hasAnimations: true) {
                        ...
                        case "RED2" : playerIndex = 1
                            whoseTurnItIs = DiceNumber == 6 ? .RED : .Green
                            redPlayerArmy[playerIndex] = MoveThatPlayers(player: redPlayerArmy[playerIndex], Army: "RED", hasAnimations: true, completionHandler:
                                                                            {
                                                                                //self.redPlayerArmy[playerIndex].player.animationPlayer(forKey: self.KIDKey)?.stop()
                                                                                print("STOPPED")
                    ...
                    case .Green:
                        ...
                        case "GREEN1" : playerIndex = 0
                            ...
                            greenPlayerArmy[playerIndex] = MoveThatPlayers(player: greenPlayerArmy[playerIndex], Army: "GREEN", hasAnimations: true, completionHandler: {})
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor)
    {
        ...
        if let planeAnchor = anchor as? ARPlaneAnchor, !boardLoaded
        {
                self.LoadLudoBoard(withNode: node, andAnchor: planeAnchor)
            boardLoaded = true
            let configuration = ARWorldTrackingConfiguration()
